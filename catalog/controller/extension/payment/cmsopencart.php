...
class ControllerExtensionPaymentCmsopencart extends Controller {
	...
	public function index()
	{
		...
		if($order_info['payment_method'] == 'Cmsopencart') {
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . 'extension/payment/cmsopencart_cms.tpl')) {
				return $this->load->view($this->config->get('config_template') . 'extension/payment/cmsopencart_cms.tpl', $data);	
			} else {
				return $this->load->view('extension/payment/cmsopencart_cms.tpl', $data);
			}		
		}
	...
	private function process_cs()
	{
		...
		$data['cs_module'] = $this->config->get('cmsopencart_module');
		$data['cs_vanityurl'] = $this->config->get('cmsopencart_cs_vanityurl');
		$data['cs_access_key'] = $this->config->get('cmsopencart_cs_access_key');
		$data['cs_secret_key'] = $this->config->get('cmsopencart_cs_secret_key');
		$data['cs_merchant_trans_id'] = rand(10,90).'ORD'.$order_info['order_id'];
		$data['currency'] = $this->config->get('cmsopencart_currency');
	...
	private function process_cms() {	
		...
		$data['merchant'] = $this->config->get('cmsopencart_cms_merchant');
		
		 
		
		
		if($this->config->get('cmsopencart_module')=='Test')
			
			$data['action'] = $this->config->get('cmsopencart_cms_testurl');
		else
		  
		    $data['action'] = $this->config->get('cmsopencart_cms_liveurl');
			
		

		             
		$data['toid'] = $this->config->get('cmsopencart_cms_merchant');
		$data['key'] = $this->config->get('cmsopencart_cms_workingkey');
		$data['testurl'] = $this->config->get('cmsopencart_cms_testurl');
		$data['liveurl'] = $this->config->get('cmsopencart_cms_liveurl');
		$data['totype'] = $this->config->get('cmsopencart_cms_partner_name');
		$data['cardtype'] = '';
		$data['paymenttype'] = '';
		$data['reservedField1'] = '';
		$data['reservedField2'] = '';
		$data['pctype'] = '1_1|1_2';
		$data['partenerid'] = $this->config->get('cmsopencart_cms_partner_id');
		$data['ipaddr'] = $this->config->get('cmsopencart_cms_ipaddress');
		...
		/************** Variables ***************/
			$amount = $data['amount'];
			$toid =  $this->config->get('cmsopencart_cms_merchant');
			$workingkey   =  $this->config->get('cmsopencart_cms_workingkey');
			$totype = $this->config->get('cmsopencart_cms_partner_name');
...
$str = $trackingid .'|'. $desc .'|' . $amount .'|' . $status .'|'. $workingkey;
		...
		{ 	
			...
			{	
				...
				$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('cmsopencart_order_status_id'),$_POST['status'],true);
			...
			{	
				...
				$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('cmsopencart_order_fail_status_id'),$_POST['status'],true);					
				if($this->session) $this->session->data['error'] = "Transaction Failed. Please try again later!";
				if($this->response) $this->response->redirect($this->url->link('checkout/checkout', '', 'SSL'));
			}
			elseif ($_POST['status'] == 'C') 
			{
				
				$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('cmsopencart_order_cancel_status_id'),$_POST['status'],true);					
				if($this->session) $this->session->data['error'] = "Thank you for shopping with us. However, the transaction has been cancelled.";
				if($this->response) $this->response->redirect($this->url->link('checkout/checkout', '', 'SSL'));
			}
	...
	public function callback_cms() {
		if (isset($this->request->post['key']) && ($this->request->post['key'] == $this->config->get('cmsopencart_cms_merchant'))) {
			$this->language->load('extension/payment/cmsopencart');
			$this->load->model('checkout/order');
